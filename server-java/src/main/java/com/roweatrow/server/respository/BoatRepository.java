...
public interface BoatRepository extends CrudRepository<Boat, Long> {
  ...
  @Query(
      nativeQuery = true,
      value =
          "SELECT boat.* FROM boat boat WHERE ?1 IN (seat_1, seat_2, seat_3, seat_4, seat_5, seat_6, seat_7, seat_8)")
  List<Boat> retrieveAllByAthleteId(long athleteId);
}
